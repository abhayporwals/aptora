...
const TopGainers = () => {
    const { topGainers, isLoading, error } = useDashboardData();
    ...
    if (error) {
        return (
            <div className="p-6 rounded-2xl backdrop-blur-sm bg-card/30 border border-white/5 shadow-xl">
                <p className="text-red-500">Error loading Top Gainers: {error}</p>
            </div>
        );
    }
                ...
                {topGainers.map((item, index) => (
                    ...
                    >
                        ...
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-xs text-green-500 px-2 py-1 rounded-full bg-green-500/10">
